...
const fallbackFruits=["truskawki","maliny","porzeczka czarna i czerwona","wiśnie","śliwki","aronia","jabłko przemysłowe","jabłko na obierkę"].map((name,i)=>({name,image_url:i>5?"/images/jablka.jpg":"/images/owoce-kafle.png",sort_order:i,is_visible:true}));
...
function App(){
 ...
 const load=async()=>{
   if(!supabase) return;
   const [st,se,fr,di,ga]=await Promise.all([
 ...
 if(location.pathname.includes("/admin")) return <Admin settings={settings} sections={sections} fruits={fruits} directions={directions} gallery={gallery} reload={load}/>;
...
function Header({sections}){return <header className="header"><a className="brand" href="/#start"><img src="/images/logo-jablko.png" alt="AGRO-MAR logo"/><span><strong>AGRO-MAR</strong><small>Mariusz Bańka Sp. z o.o.</small></span></a><nav><a href="/#start">Strona główna</a>{sections.map(s=><a key={s.slug} href={"/#"+s.slug}>{s.menu_label}</a>)}<a href="/admin" className="adminLink">Admin</a></nav></header>}
...
function Contact({sec,settings}){return <section id={sec.slug} className="section contact split"><div className="sectionTitle"><p className="label">{sec.menu_label}</p><h2>{sec.title}</h2></div><div className="contactCard"><Text body={sec.body}/><a href={"tel:"+settings.phone}>{settings.phone}</a><a href={"mailto:"+settings.email}>{settings.email}</a><span>{settings.location}</span></div></section>}
...
function Admin({settings,sections,fruits,directions,gallery,reload}){
